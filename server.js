...
>>>>>>> eb882a0204c14704405ce49c905d6a787de18b69
...
app.post("/api/register", async (req, res) => {
  ...
  try {
    ...
    await Profile.create({ user_id: user._id, name: user.name, email: user.email });
...
app.post("/api/forgot-password", async (req, res) => {
  ...
  try {
    ...
    await User.updateOne({ email }, { reset_token: token, reset_expires: expires });
    const resetLink = `http://localhost:${PORT}/reset.html?token=${token}`;
    await transporter.sendMail({
      from: `"Job Tracker" <${process.env.EMAIL_USER}>`,
      to: email, subject: "Reset your Job Tracker password",
...
app.post("/api/reset-password", async (req, res) => {
  ...
  try {
    ...
    const hashed = await bcrypt.hash(password, 12);
    await User.updateOne({ _id: user._id }, { password: hashed, reset_token: null, reset_expires: null });
    res.json({ success: true });
  } catch (err) { console.error(err); res.status(500).json({ error: "Server error." }); }
});
...
app.post("/api/jobs", requireAuth, async (req, res) => {
  ...
  try {
    const job = await Job.create({
      user_id: req.session.userId, company, role,
  ...
  } catch (err) { res.status(500).json({ error: "Failed to save job." }); }
});
...
app.delete("/api/jobs/:id", requireAuth, async (req, res) => {
  try {
    await Job.deleteOne({ _id: req.params.id, user_id: req.session.userId });
    res.json({ success: true });
